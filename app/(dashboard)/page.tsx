...
import { UserCircle, AudioLines, Database, Music, Sparkles, ArrowUpRight, FolderKanban, Clapperboard } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const chatModelCount = CHAT_MODELS.length;

  const modules = [
    {
      href: "/ai",
      title: "文本工具",
      description: "多模型文本对话，支持附件理解。",
    ...
    },
    {
      href: "/media/image",
      title: "媒体工具",
      description: "文字生成图片与短视频，适合封面、镜头与视觉素材。",
      icon: <Clapperboard className="h-5 w-5" />,
      tone: "blue",
      stats: [
        ["图片", 1],
    ...
    <div className="flex flex-col gap-6 md:gap-8">
      <section className="grid grid-cols-1 gap-3 min-[520px]:grid-cols-2 xl:grid-cols-5">
        <MetricCard
          ...
          description="Claude · Gemini · GPT · DeepSeek"
          icon={<Sparkles className="h-5 w-5" />}
          tone="plum"
        />
        <MetricCard
            ...
            <div className="border-b border-[var(--oa-card-head-border)] bg-[var(--oa-paper-soft)] px-5 py-5 text-[var(--oa-ink)] md:px-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-[var(--oa-muted)]">四大核心能力按创制流程排布，点开即可进入对应工作台。</p>
                </div>
              </div>
            </div>
