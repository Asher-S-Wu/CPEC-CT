...
import { Sidebar } from "@/components/media/layout/media-sidebar";
import { requirePageSession } from "@/lib/auth";

async function MediaLayoutContent({ children }: { children: ReactNode }) {
...
export default function MediaLayout({ children }: { children: ReactNode }) {
  return (
    <Suspense fallback={<ConsoleLoadingShell variant="section" sectionItems={2} />}>
      <MediaLayoutContent>{children}</MediaLayoutContent>
    </Suspense>
  );
}
