...
import {
  getMinimaxAnthropicContentBlocks,
  normalizeMinimaxProviderState,
...
function normalizePersistedProviderState(providerState) {
  ...
  const minimaxState = normalizeMinimaxProviderState(providerState);
  if (minimaxState) return minimaxState;
...
function hasMeaningfulModelProgress(message, { content, parts, thinkingTimeline, citations, tools, artifacts, providerState } = {}) {
  ...
  if (getMinimaxAnthropicContentBlocks(providerState || message.providerState)) {
    return true;
  }
