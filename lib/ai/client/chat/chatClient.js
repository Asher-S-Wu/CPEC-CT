...
function normalizePersistedProviderState(providerState) {
  ...
  const sanitized = sanitizeJsonValue(providerState);
  const encryptedContent = providerState?.arkChatCompletions?.thinking?.encrypted_content;
  if (
    sanitized?.arkChatCompletions?.thinking
    && typeof encryptedContent === "string"
    && encryptedContent
  ) {
    sanitized.arkChatCompletions.thinking.encrypted_content = encryptedContent;
  }
  return sanitized;
}
