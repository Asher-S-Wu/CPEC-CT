...
import { isDataImageUrl, isStoredImageUrl } from "@/lib/ai/shared/messageImage";
...
export function createChatAppActions({
  ...
  const onEditingImageSelect = (img) => {
    ...
    uploadStoredFile(img.file, { scope: "chat", model }).then((storedFile) => {
      setEditingImage((prev) => (
        prev?.uploadId === uploadId
          ? { ...prev, uploadStatus: "ready", fileUrl: storedFile.url, fileId: storedFile.fileId, errorMessage: "" }
          : prev
      ));
    }).catch((error) => {
      setEditingImage((prev) => (
        prev?.uploadId === uploadId
          ? { ...prev, uploadStatus: "error", fileUrl: null, fileId: null, errorMessage: error?.message || "未知错误" }
          : prev
      ));
  ...
  const handleSendFromComposer = async ({ text, attachments }) => {
    ...
    for (const image of uploadedImages) {
      if (image?.url && image?.mimeType) {
        userMsgParts.push({ inlineData: { fileId: image.fileId, url: image.url, mimeType: image.mimeType } });
      }
    }
  ...
  const submitEditAndRegenerate = async (index) => {
    ...
    const canKeepExistingImages = existingImageParts.length > 0 && existingImageParts.every((p) => {
      ...
      return (isStoredImageUrl(url) || isDataImageUrl(url)) && typeof mimeType === "string" && Boolean(mimeType);
    });
    ...
    try {
      if (editingImageAction === "remove") {
        ...
        if (!fileUrl || !fileId || !mimeType) throw new Error("图片还没上传完成");
        nextImageParts = [{ inlineData: { fileId, url: fileUrl, mimeType } }];
        ...
        for (const p of existingImageParts) {
          ...
          if (isStoredImageUrl(src)) {
            const fileId = typeof p?.inlineData?.fileId === "string" ? p.inlineData.fileId : "";
            if (!fileId) throw new Error("图片文件编号缺失");
            nextImageParts.push({ inlineData: { fileId, url: src, mimeType } });
            continue;
          }
          ...
          if (isDataImageUrl(src)) {
            ...
            const uploaded = await uploadStoredFile(file, { scope: "chat", model });
            nextImageParts.push({ inlineData: { fileId: uploaded.fileId, url: uploaded.url, mimeType: mime } });
          }
        }
      }
