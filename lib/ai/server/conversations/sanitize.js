...
import { normalizeMinimaxProviderState } from "@/lib/ai/shared/minimaxAnthropicState";
...
export function sanitizeMessage(message, index, { allowContentFallback = false } = {}) {
  ...
  const providerState = normalizeMinimaxProviderState(message.providerState) || sanitizeJsonValue(message.providerState);
