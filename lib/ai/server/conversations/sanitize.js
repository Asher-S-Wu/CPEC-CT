...
function sanitizeCitations(value, fieldPath) {
  ...
  for (const item of value) {
    ...
    if (favicon && favicon.length <= MAX_URL_CHARS) entry.favicon = favicon;
...
function sanitizeArtifacts(value, fieldPath) {
  ...
  for (const item of value) {
    ...
    if (!url || !isAllowedFileUrl(url)) continue;
    artifacts.push({
      url,
...
export function sanitizeMessage(message, index, { allowContentFallback = false } = {}) {
  ...
  for (const part of sourceParts) {
    ...
    if (isPlainObject(part.inlineData)) {
      ...
      if (!fileId) throw new Error(`messages[${index}].parts image fileId invalid`);
      nextPart.inlineData.fileId = fileId;
    }
    if (isPlainObject(part.fileData)) {
      ...
      if (!fileId) throw new Error(`messages[${index}].parts file fileId invalid`);
      nextPart.fileData.fileId = fileId;
    }
